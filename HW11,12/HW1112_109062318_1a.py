...
for k in range(1000):
    ...
    for i in range(100):  # the number 100 need to be fixed maximal number of trials
        ...
        if state_x==width-1 and state_y==length-1:
            ...
            #print(f"now={count_move}, best={count_move_prev}",end='\t')
            reached=True
            print('iteration',k, 'move', count_move, 'goal reached')
            break
    if count_move<count_move_prev and reached: #if the current move is better
...
# my code
print(f"UP={prev_trajectory.count(1)}, RIGHT={prev_trajectory.count(2)}, LEFT={prev_trajectory.count(3)}")

mygrid=[['.' for i in range(10)] for j in range(10)]
...
myplot(start=(9,0), path=prev_trajectory)
for row in mygrid:
    for col in row:
        print(col,end='')
    print()